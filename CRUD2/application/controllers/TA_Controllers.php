<?php
	class TA_Controllers extends CI_Controller{
		...
		public function tabel_ta($nim = NULL){
		...
		function submitTA(){
			...
			redirect ('TA_Controllers/tabel_ta/'.$this->input->post('nim'));
		}
		...
		function deleteTA($nim){
			$data['status'] = $this->aktor->getStatus();
			$data = array('id_admin' => $data['status'], 'aksi'=>'Menghapus Data Dokumen Makalah Tugas Akhir', 'tujuan'=>0);
			$this->db->insert('aktifitas',$data);
